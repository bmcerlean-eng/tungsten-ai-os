...
function SignalIcon({ type }: { type: string }) {
  if (type === "renewal_approaching") return <AlertTriangle size={12} className="text-amber-500" />;
  if (type === "expansion_ready") return <TrendingUp size={12} className="text-emerald-500" />;
  if (type === "contact_gap") return <AlertTriangle size={12} className="text-red-500" />;
...
export default function Dashboard() {
  ...
  const kpis = [
    { label: "Portfolio ARR", value: formatARR(totalARR), icon: DollarSign, color: "text-tungsten-navy", bg: "bg-tungsten-navy/5 border-tungsten-navy/20" },
    { label: "At Risk", value: atRisk.length, icon: AlertTriangle, color: "text-red-500", bg: "bg-red-50 border-red-100" },
    { label: "Needs Attention", value: needsAttention.length, icon: AlertTriangle, color: "text-amber-500", bg: "bg-amber-50 border-amber-100" },
    { label: "Renewing ≤120 Days", value: renewingSoon.length, icon: RefreshCw, color: "text-blue-500", bg: "bg-blue-50 border-blue-100" },
  ];

  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-6">
        <h1 className="text-xl font-bold text-tungsten-navy">Portfolio Command Center</h1>
        <p className="text-sm text-gray-400 mt-1">Strategic accounts — {new Date().toLocaleDateString("en-US", { dateStyle: "long" })}</p>
      </div>

      {/* KPI row */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {kpis.map(({ label, value, icon: Icon, color, bg }) => (
          <div key={label} className={`bg-white border rounded-xl p-4 shadow-sm ${bg}`}>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        {/* Account table */}
        <div className="xl:col-span-2 bg-white border border-tungsten-border rounded-xl overflow-hidden shadow-sm">
          <div className="px-5 py-4 border-b border-tungsten-border">
            <h2 className="text-sm font-semibold text-tungsten-navy">Account Portfolio</h2>
          </div>
                  ...
                  return (
                    <tr key={account.id} className="border-b border-tungsten-border/60 hover:bg-tungsten-surface transition-colors">
                      <td className="px-4 py-3">
                        ...
                        <div className="text-xs text-gray-400">{account.region} · {account.industry.split(" ")[0]}</div>
                      </td>
                      <td className="px-4 py-3 text-gray-700 font-semibold">{formatARR(account.arrTotal)}</td>
                      ...
                      <td className="px-4 py-3">
                        <Link
                          ...
                          className="text-xs text-tungsten-navy hover:text-tungsten-blue font-semibold transition-colors"
                        >
                          View →
                        </Link>
                      </td>
        ...
        <div className="bg-white border border-tungsten-border rounded-xl overflow-hidden shadow-sm">
          <div className="px-5 py-4 border-b border-tungsten-border">
            <h2 className="text-sm font-semibold text-tungsten-navy">Active Signals</h2>
            <p className="text-xs text-gray-400 mt-0.5">{allSignals.length} requiring attention</p>
          </div>
          <div className="divide-y divide-tungsten-border/60">
            {allSignals.map((signal, i) => (
              <div key={i} className="px-5 py-3 hover:bg-tungsten-surface transition-colors">
                <div className="flex items-start gap-2">
                  <SignalIcon type={signal.type} />
                  <div className="flex-1 min-w-0">
                    <Link href={`/accounts/${signal.accountId}`} className="text-xs font-semibold text-tungsten-navy hover:text-tungsten-blue">
                      {signal.accountName}
                    </Link>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-6">
        {[
          ...
          <div key={label} className="bg-white border border-tungsten-border rounded-xl p-5 shadow-sm">
            <div className="text-xs text-gray-400 font-medium mb-1">{label}</div>
            <div className="text-xl font-bold text-tungsten-navy mb-1">{value}</div>
            <div className="text-xs text-gray-400 mb-3">{target}</div>
